...
//  HW2Xcode
//
//  Created by Rachel Wofford on 2/18/18.
...
//Exercise 1.8 completed by Rachel Wofford
void do_remove_entry(Phone_Directory& the_directory) // Exercise 1.8: please complete the function do_remove_entry - Ed/Kent
{
    ...
    //Call remove-entry function to overwrite name is array and assign return value to removeName
    string removeName = the_directory.remove_entry(name);
    //If the name is in the array, it will have been removed by the previous function
    if (removeName != "") {
        //Output the removal to the user
        cout << "The name " << removeName << " was removed. " << endl;
    }
    //If name is not in the array, inform the user
    else {
        cout << name << " is not in the directory\n";
    }
