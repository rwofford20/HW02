...
//1.7 Done by Rachel Wofford
{
    for(int j=0; j < size - 1; j++)
    {
        //Call function find to determine if name is already in directory
        if (name == the_directory[j].get_name())
        {
            //Overwrite entry from array
            string nameNumber = the_directory[j].get_number();
    ...
    //If name is not already in directory, return an empty string
    //This functions like an else statement because it won't be reached if the if statement is true
        //And, it also prevents the possibility of reaching the end of a non-void statement
    return "";
}
